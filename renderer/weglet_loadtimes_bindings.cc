...
// Chrome's own version of this also feeds a UseCounter for its own
// telemetry; weglet has none, so this just returns the stored value.
void LoadtimesGetterHelper(v8::Local<v8::Name> name,
                           const v8::PropertyCallbackInfo<v8::Value>& info) {
  info.GetReturnValue().Set(info.Data());
}

void CSIGetterHelper(v8::Local<v8::Name> name,
                     const v8::PropertyCallbackInfo<v8::Value>& info) {
  info.GetReturnValue().Set(info.Data());
}
...
// static
void WegletLoadTimesBindings::LoadTimesCallback(
    const v8::FunctionCallbackInfo<v8::Value>& info) {
  v8::Isolate* isolate = info.GetIsolate();
  WegletLoadTimesBindings* bindings = nullptr;
...
// static
void WegletLoadTimesBindings::CSICallback(
    const v8::FunctionCallbackInfo<v8::Value>& info) {
  v8::Isolate* isolate = info.GetIsolate();
  WegletLoadTimesBindings* bindings = nullptr;
  if (gin::ConvertFromV8(isolate, info.Data(), &bindings) && bindings) {
...
// static
void WegletLoadTimesBindings::Install(v8::Local<v8::Context> context) {
  ...
  auto* bindings = cppgc::MakeGarbageCollected<WegletLoadTimesBindings>(
...
WegletLoadTimesBindings::WegletLoadTimesBindings() = default;

WegletLoadTimesBindings::~WegletLoadTimesBindings() = default;

gin::ObjectTemplateBuilder WegletLoadTimesBindings::GetObjectTemplateBuilder(
    v8::Isolate* isolate) {
  return gin::Wrappable<WegletLoadTimesBindings>::GetObjectTemplateBuilder(isolate);
}

v8::Local<v8::Value> WegletLoadTimesBindings::GetLoadTimes(
    v8::Isolate* isolate) {
  ...
  // request_time historically meant "the time the request to load the page
  // was received" -- now called navigation start. Kept for backward compat.
  ...
  auto define_prop = [=](std::string_view name, v8::Local<v8::Value> value) {
    ...
    return load_times
        ->SetNativeDataProperty(ctx, name_str, LoadtimesGetterHelper,
                                EmptySetter, value)
        .FromMaybe(false);
  };
...
v8::Local<v8::Value> WegletLoadTimesBindings::GetCSI(v8::Isolate* isolate) {
  blink::WebLocalFrame* frame = blink::WebLocalFrame::FrameForCurrentContext();
  if (!frame) {
    return v8::Null(isolate);
  }
  ...
  auto define_prop = [=](std::string_view name, v8::Local<v8::Value> value) {
    ...
    return csi
        ->SetNativeDataProperty(ctx, name_str, CSIGetterHelper, EmptySetter,
                                value)
        .FromMaybe(false);
  };
...
const gin::WrapperInfo* WegletLoadTimesBindings::wrapper_info() const {
  return &kWrapperInfo;
}
